...
public class DomainExceptionValidation(string error) : Exception(error)
{
    ...
    {
        if (hasError) throw Activator.CreateInstance<TException>();
    }

    public static void When<TException>(bool hasError, Func<TException> throwFunc)
        where TException : DomainExceptionValidation
    {
        if (hasError) throw throwFunc();
    }
}